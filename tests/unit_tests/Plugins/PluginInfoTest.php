...
class PluginInfoTest extends TerminusTestCase
{
    ...
    public function testFailCompatibleVersionlessComposer()
    {
        ...
        $this->expectExceptionMessage('Composer.json must contain a "compatible-version" field in "extras/terminus"');
        new PluginInfo($dir);
    }
    ...
    /**
     * Tests PluginInfo failing to construct because the composer.json does not
     * contain a Terminus property in its extras
     */
    ...
    public function testFailInvalidNamespace()
    {
        $dir = $this->paths[7];
        $this->expectException(TerminusException::class);
        $this->expectExceptionMessage('The namespace "OrgName\\\\PluginName" in the composer.json autoload PSR-4 '.
            'section must end with a namespace separator. Should be "OrgName\\\\PluginName\\\\"');
        new PluginInfo($dir);
    }
